...
class Engine(object):
    ...
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        depth_mapping_3d_0_var = torch.autograd.Variable(self.state['depth_mapping_3d_0'], requires_grad=False)
        depth_mapping_3d_1_var = torch.autograd.Variable(self.state['depth_mapping_3d_1'], requires_grad=False)

        if training:
            self.state['output'] = model(input_var, depth_var, depth_mapping_3d_0_var, depth_mapping_3d_1_var)
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (color, depth, label, label_weight, depth_mapping_3d) in enumerate(data_loader):
            ...
            self.state['input'] = self.state['input'].cuda(device = 0, async=True)
            self.state['depth'] = self.state['depth'].cuda(device = 1, async=True)
            self.state['target'] = self.state['target'].cuda(device = 0, async=True)
            self.state['target_weight'] = self.state['target_weight'].cuda(device = 0, async=True)
            self.state['depth_mapping_3d_0'] = self.state['depth_mapping_3d'].cuda(device = 0, async=True)
            self.state['depth_mapping_3d_1'] = self.state['depth_mapping_3d'].cuda(device = 1, async=True)
    ...
    def save_checkpoint(self, state, is_best, filename='checkpoint.pth.tar'):
        ...
        if self.state['iteration'] % self.state['save_iter'] == 0:
            filename_my = os.path.join(self.state['save_model_path'], 'checkpoint_{0}({1}).pth.tar'.format(self.state['epoch']+1, self.state['iteration']))
            torch.save(state, filename_my)
        # my add
